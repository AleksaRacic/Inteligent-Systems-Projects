...
class GameState:
    ...
    def is_agent_win(self, _agent_id):
        actions_len = [len(self.get_legal_actions(agent_id)) for agent_id in range(len(self.agents))]
        if not any(actions_len[:_agent_id] + actions_len[_agent_id+1:]) and actions_len[_agent_id]:
            return True
        elif not any(actions_len):
            if self.last_agent_played_id is not None:
                return self.last_agent_played_id == _agent_id
        else:
            return False

    def is_agent_loss(self, _agent_id):
        actions_len = [len(self.get_legal_actions(agent_id)) for agent_id in range(len(self.agents))]
        if any(actions_len[:_agent_id] + actions_len[_agent_id+1:]) and not actions_len[_agent_id]:
            return True
        elif not any(actions_len):
            if self.last_agent_played_id is not None:
                return self.last_agent_played_id != _agent_id
